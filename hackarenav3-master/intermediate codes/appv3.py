...
class MedicalImageAnalyzer:
    ...
    def analyze_image_simple(self, image):
        """Simple analysis of image using the image agent"""
        try:
            ...
            chat_response = self.user_proxy.initiate_chat(
                self.image_agent,
                message="Describe what is in this image in 2 sentences?",
                image=img_byte_arr
            )
            ...
            st.error(f"Error in simple image analysis: {str(e)}")
            return None

    def analyze_medical_image(self, image):
        """Detailed medical analysis of image using the image agent"""
        try:
            # Convert PIL Image to bytes for the agent
            img_byte_arr = io.BytesIO()
            image.save(img_byte_arr, format=image.format or 'PNG')
            ...
            st.error(f"Error in medical image analysis: {str(e)}")
            return None
    ...
    if page == "Image Analysis":
        ...
        if uploaded_file is not None:
            ...
            with st.spinner("Analyzing image..."):
                if analysis_type == "Simple Description":
                    analysis_result = analyzer.analyze_image_simple(image)
                else:
